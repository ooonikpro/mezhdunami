<template>
    ...
    >
        <template #title> Спасибо, вы записаны. </template>

        <div class="form-summary mb-32">
            <div class="h3">{{ props.formData.name }},</div>
            <div class="h3 mb-24">{{ props.formData.phone }}</div>

            <div class="label h4 mb-8">День записи:</div>
            <div class="h3 mb-24">
                {{ getLocalizedFullDate(props.formData.date) }}
            </div>

            <div class="label h4 mb-8">Процедуры:</div>
            <div class="h3 mb-24">
                {{ getNames(props.formData.procedures) }}
            </div>

            <div class="label h4 mb-8">Время {{ procedureLabel }}:</div>
            <div class="h3 mb-24">
                {{ getTotalDurationLocalized(props.formData.procedures) }}
            </div>

            <div class="label h4 mb-8">Ориентировочная стоимость:</div>
            <div class="h3">
